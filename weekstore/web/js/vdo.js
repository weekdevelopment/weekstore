...
function playPause() {
    if (video.paused) {
        video.play(); play.style.display = "none"; pause.style.display = "inline-block";
    } else {
        video.pause(); play.style.display = "inline-block"; pause.style.display = "none";
    }
}
//정지함수
...
function mutePlayer(state) { //음소거토글함수
    if (state == "mute") {
        video.volume = 0; mute.style.display = "none";
        unmute.style.display = "inline-block"; volTxt.innerText = "0%";
    } else if (state == "unmute") {
        video.volume = 1; mute.style.display = "inline-block";
